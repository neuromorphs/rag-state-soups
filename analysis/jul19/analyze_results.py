...
for idx, lcond in enumerate(df["layer_condition"].unique()):
    x = df[df["layer_condition"] == lcond]
    x = x.sort_values(["sample_id", "correct"]).drop_duplicates(subset=["sample_id"], keep="first")
    plt.plot(x["sample_id"], x["correct"] + idx, label=lcond)
plt.legend()
plt.savefig("lcond.png", dpi=300)
plt.close()
